...
def pngPath(page_id):
	...
	if os.path.getmtime(thmbnail) < time.mktime(time.strptime(str(current.db(current.db.page.id==page_id).select().first().modified_on), "%Y-%m-%d %H:%M:%S")) :
		im = Image.new("RGB",(168,120))
		draw = ImageDraw.Draw(im)
		draw.rectangle((0,0) + im.size, fill=(250,250,250))

		for box in current.db(current.db.container_box.page_id==page_id).select() :
